...
const ContactForm = () => {
  ...
  const sendMail = (e) => {
    ...
    setLoading(true);
    emailjs
      .sendForm("service_4apvbgc", "template_u01iaho", form.current, "ixjwKAv_pxSFAN7B9")
      .then(
        () => { setLoading(false); notifyMessageSuccess(); cleanForm(); },
        () => { setLoading(false); notifyMessageError(); },
      );
  };
  ...
  return (
    <div className="glass-card px-[100px] py-[60px] max-[768px]:p-5">
      ...
      <div className="flex gap-[10px] justify-end">
        ...
        >
          <IoLogoGithub size={50} className="cursor-pointer max-[768px]:w-10 max-[768px]:h-10" />
        </a>
        <a
          ...
          className="text-[var(--icon-color)] hover:-translate-y-[2px]"
        >
          <FaLinkedin size={50} className="cursor-pointer max-[768px]:w-10 max-[768px]:h-10" />
        </a>
      </div>
      ...
      <form onSubmit={sendMail} ref={form} className="flex flex-col gap-0">
        ...
        <div className="flex flex-col">
          <label className="text-[var(--text-color)] text-left w-full">{t("contact.name")}</label>
          <input
            type="text"
            name="name"
            value={name}
        ...
        {/* Email */}
        <div className="flex flex-col">
          <label className="text-[var(--text-color)] text-left w-full">{t("contact.email")}</label>
          <input
            type="email"
            name="email"
            value={email}
        ...
        {/* Mensaje */}
        <div className="flex flex-col">
          <label className="text-[var(--text-color)] text-left w-full">{t("contact.message")}</label>
          <textarea
            name="message"
            value={message}
